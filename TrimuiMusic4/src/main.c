...
static int skip_to_track(Browser *browser, int direction) {
...
static void load_and_play(const char *path, RingBuffer *rb, Decoder **dec,
    ...
    *is_paused = 0;
    ...
    lyrics_load_for_audio(lyrics, current_path);
}
...
int main(int argc, char **argv) {
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (screen == SCREEN_BROWSER) {
                switch (act) {
                    ...
                    case ACTION_QUIT: running = 0; break;
                    case ACTION_CONFIRM: {
        ...
        if (screen == SCREEN_BROWSER) {
            ...
            SDL_Rect list_area = { 40, 130, SCREEN_W - 80, SCREEN_H - 180 };
